      ...
      {lockers.map(locker => {
        ...
        return (
          <div key={locker.locker_id} style={{ background: '#12121e', border: '1px solid #2a2a44', borderRadius: 8, padding: 12, marginBottom: 8 }}>
            <div style={{ fontSize: 13, color: '#ff6b6b', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>🔐 ロッカー{locker.locker_number}（{locker.slot_count}スロット・{locker.lock_type === 'key' ? '鍵式' : '暗証番号'}）{locker.posLabel} — 入替/補充</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
              {slots.map(sl => (
                <SlotCard key={sl.slot_id} slot={sl} mode="edit"
                  onAction={(s) => setModalSlot(s)} />
              ))}
            </div>
