...
export default function DataSearch() {
  ...
  async function saveAll() {
    ...
    try {
      ...
      setAllReadings(fresh); setEdits({}); setEditingRow(null)
      setSaveMsg(`✅ ${entries.length}件保存しました`)
    } catch(e) { setSaveMsg('❌ 保存エラー: '+e.message) }
    setSaving(false)
  }
  ...
  return (
    <div style={{padding:'16px 14px 100px'}}>
      ...
      {filtered.map(r => {
        ...
        return (
          <div key={r._idx} style={{...S.card, marginBottom:8,
            ...
            {isEditing ? (
              <div>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:10}}>
                  <div>
                    ...
                    <div style={{fontSize:11, color:'var(--muted)', marginTop:2}}>{r.read_time?.slice(0,10)}</div>
                  </div>
              ...
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}
                ...
                <div>
                  ...
                  <div style={{fontSize:12, color:'var(--muted)', marginTop:1}}>{r.prize_name||'景品名なし'}</div>
                </div>
                <div style={{color:'var(--accent4)', fontSize:18}}>✏️</div>
              </div>
            )}
          </div>
        )
