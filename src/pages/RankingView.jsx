...
export default function RankingView() {
        ...
        if (br.length >= 2) {
          ...
          if (!isNaN(prev) && !isNaN(curr) && curr >= prev) {
            ...
            stats.push({
              ...
              diff, sales: diff * parseNum(b.play_price||'100'),
              prize_name: br[br.length-1].prize_name || '-',
              read_time: br[br.length-1].read_time?.slice(0,10),
              noData: false
            })
          } else {
            stats.push({ full_booth_code: b.full_booth_code, machine_name: m.machine_name,
              diff:0, sales:0, prize_name: br[br.length-1].prize_name||'-',
              read_time: br[br.length-1].read_time?.slice(0,10), noData: true })
          }
        } else if (br.length === 1) {
          stats.push({ full_booth_code: b.full_booth_code, machine_name: m.machine_name,
            diff:0, sales:0, prize_name: br[0].prize_name||'-',
            read_time: br[0].read_time?.slice(0,10), noData: true })
        } else {
          stats.push({ full_booth_code: b.full_booth_code, machine_name: m.machine_name,
            diff:0, sales:0, prize_name:'-', read_time:'-', noData: true })
        }
      }
    }
    setBoothStats(stats)
  ...
  const top3 = [...withData].sort((a,b) => b.sales-a.sales).slice(0,3)
  const worst3 = [...withData].sort((a,b) => a.sales-b.sales).slice(0,3)
  const totalSales = withData.reduce((s,b) => s+b.sales, 0)

  if (loading) return (
    <div className="container" style={{paddingTop:40,textAlign:'center'}}>
      <p>集計中...</p><p style={{fontSize:12,color:'#999',marginTop:8}}>少々お待ちください</p>
    </div>
  )
