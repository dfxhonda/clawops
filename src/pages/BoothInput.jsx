...
import { getBooths, getLastReading, getMachines, parseNum } from '../services/sheets'
...
export default function BoothInput() {
  ...
  useEffect(() => {
    getBooths(machineId).then(async bs => {
      setBooths(bs)
      const readings = {}
      for (const b of bs) readings[b.booth_id] = await getLastReading(b.booth_id)
      ...
      for (const b of bs) {
        ...
        if (draft) {
          restored[b.booth_id] = { in_meter:draft.in_meter, out_meter:draft.out_meter,
            prize_restock:draft.prize_restock_count, prize_stock:draft.prize_stock_count, prize_name:draft.prize_name }
  ...
  // IN差分
  const inVal = inp.in_meter !== undefined && inp.in_meter !== '' ? parseNum(inp.in_meter) : null
  ...
  // OUT差分
  const outVal = inp.out_meter !== undefined && inp.out_meter !== '' ? parseNum(inp.out_meter) : null
  ...
  function handleSave() {
    ...
    saveDraft({
      ...
      in_meter: inp.in_meter, out_meter: inp.out_meter||'',
        ...
        <div style={{marginBottom:16}}>
          ...
          {inDiff !== null && (
            <div className={`diff ${inAbnormal?'warning':'normal'}`} style={{marginTop:6}}>
              差分: +{inDiff.toLocaleString()}回 / ¥{(inDiff*price).toLocaleString()}
