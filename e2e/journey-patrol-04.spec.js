...
test.describe('J-PATROL-04: 前回値プリフィル・差分保存', () => {
  test('フォームが前回読みで初期化される', async ({ page }) => {
    ...
    await expect(page.locator('#field-in-meter')).toHaveValue('50000')
    await expect(page.locator('#field-out-meter')).toHaveValue('45000')
    await expect(page.locator('#field-stock')).toHaveValue('20')
    await expect(page.locator('#field-restock')).toHaveValue('0')
