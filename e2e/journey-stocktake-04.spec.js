...
test.skip('J-STOCKTAKE-04a: 入力値に対応した乖離率が色付きで表示される', async ({ page }) => {
...
test.skip('J-STOCKTAKE-04b: 30%超え入力でキャンセル → 入力値がリセットされる', async ({ page }) => {
  await setupAuth(page)
  await setupBaseMocks(page)
...
test.skip('J-STOCKTAKE-04c: 30%超え入力で確認 → 保存が実行される', async ({ page }) => {
  await setupAuth(page)
